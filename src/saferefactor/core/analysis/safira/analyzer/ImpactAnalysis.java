...
//		print();
	...
	public void walkingInASourceProgram() {
		...
		//seta os methodInv que s�o do tipo This.
		for (Class sc : sourceClasses.values()) {
	...
	public void fixMetAndFieldSource() {
		...
		//Ajeitar as chamadas de m�todos 
		for (Class sc : sourceClasses.values()) {
			List<Method> methods = sc.getMethods();
			for (Method method : methods) {
				List<Method> methodInvoc = method.getMethodInvoc();
				for (Method mi : methodInvoc) {
					//verificar se o m�todo chamado est� na mesma classe
				//	if (mi.getClassFullName().equals(method.getContainsClass().getFullName())) {
					
						//verificar se a classe contem o m�todo e ele � herdado
	...
	private HashMap<String, Method> makeGraph(HashMap<String, Method> list, Method m) {
		...
		for (Method method : methodInvoc) {
			...
			if (mi != null) {
				...
				//tamb�m chama os m�todos herdados
...
//			//pq eu nao considero os herdados? e no caso de uma mudan�a de hierarquia? (ver paper de chianti)
//			if (!m.isInherited()) {
//				if (containsMethodNotInherited(targetMethods, m) == null) {
//					removedMethods.add(m);
//					putInListIntersection(m, sourceMethods);
...
//				Method methodSource = targetMethods.get(method.getFullName());
	...
	private void getChangedMethods() {
		...
		for (Method method : sourceMethods.values()) {
			...
			if (method.getFullName().contains("ClusterName")) {
					System.out.println("Entrouu ClusterName "+method.getFullName());
				}
				...
				if (method.getDeclaringClass().getFullName().equals(targetMethod.getDeclaringClass().getFullName()) &&
					...
					if (method.getFullName().contains("ClusterName.valueOf")) {
						System.out.println("tem o target e mudou? "+changeMethodBody);
					}
					if (changeMethodBody ||
...
//						fileIntersection.add((m.getSignatureRandoop()));
			...
			//se o retorno n�o for um objeto do projeto, e sim um tipo primitivo ou objeto de um jar:
			}else {
				if (isSafeMethod(m))
					fileIntersectionAux.put(m.getFullName(),m);
//					fileIntersection.add((m.getSignatureRandoop()));
...
//		Class ctarget = targetClasses.get(type);
	...
	private List<Method> getDependecies(Method m, HashMap<String,Method> fileIntersection) {
		...
		//o tipo de retorno tamb�m � uma depend�ncia para o Randoop gerar testes
...
//			if (c.getFullName().equals(name) ) {
				...
				//descomentar
				//se nao tiver construtor � pq � interface
				if (constructors.size() == 0) {
...
//			/*     */ 
	...
	public void print() {
		System.out.println("M�todos impactados");
...
//			}
		...
		System.out.println();
		System.out.println("M�todos adicionados");
		for (Method method : newMethods) {
			System.out.println(method.toString());
		}
		System.out.println();
		System.out.println("M�todos removidos");
		for (Method method : removedMethods) {
			System.out.println(method.toString());
		}
		System.out.println();
		System.out.println("M�todos modificados");
...
//		String target = "/Users/melmongiovi/Downloads/subjects/"+sub+"/JHotDraw_"+sub+"_AFTER/";
		...
		String source = "C:/Users/Felipe/Documents/resultados/T476BEFORERENAME";
		String target = "C:/Users/Felipe/Documents/resultados/T476AFTERRENAME";
