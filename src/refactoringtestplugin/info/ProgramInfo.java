			 ...
			 public boolean visit(TypeDeclaration td) {
				 td.accept(new ASTVisitor() {
						...
						public boolean visit(FieldDeclaration node) {
							Field f = new Field();
							String signature = node.fragments().toString();
							...
							for (Object bd : node.modifiers()) {
								String mod = bd.toString();
			         ...
			         //by add more visit method like the following below, then all king of statement can be visited.
				 		...
				 		Type returnType2 = md.getReturnType2();
				 		if (returnType2 != null) {
...
//					 		System.out.println(node.toString());
					 		...
					 		for (SingleVariableDeclaration md : parameters) {
					 			String signature = md.toString();
//					 			signature = signature.replace("[", "");
//					 			signature = signature.replace("]", "");
					 			if (signature.contains(" ")) {
					 				m.addParameter(signature.substring(0,signature.indexOf(" ")));
					 			} else {
					 		...
					 		for (Statement statement : statements) {
								statement.accept(new ASTVisitor() {
									public boolean visit(VariableDeclarationStatement es) {
										...
										System.out.println("variable declaration: "+es.toString());
										...
										Expression expression = es.getExpression();
						 				expression.accept(new ASTVisitor() {
						 					public boolean visit(Assignment e) {
						 						Expression leftHandSide = e.getLeftHandSide();
						 						Expression rightHandSide = e.getRightHandSide();
						 						Operator operator = e.getOperator();
						 						Statement_ st = new AssignmentSt(e.toString(),leftHandSide.toString(), rightHandSide.toString(), operator.toString());
						 						m.addStatement(st);
						 						System.out.println("assigment "+e.toString());
						 						return false;
						 					}
						 				});
										return false;
						 			...
						 			public boolean visit(ReturnStatement es) {
						 				...
						 				expression.accept(new ASTVisitor() {
						 					public boolean visit(NumberLiteral e) {
						 						m.addStatement(new ReturnSt(e.toString()));
...
//						 						m.addStatement("super.f");
						 					...
						 					public boolean visit(SimpleName e) {
						 						m.addStatement(new ReturnSt(e.toString()));
						 						System.out.println("simple name: "+e.toString());
//						 						m.addStatement("f");
						 						return false;
						 					}
...
//							 									}
							 									m.addStatement(new ReturnSt(e3.toString()));
							 								} else {
							 									m.addStatement(new ReturnSt(e3.toString()));
								 								...
								 								e3.accept(new ASTVisitor() {
								 									public boolean visit(ThisExpression e4) {
										 								if (e4.getQualifier() != null) {
										 									...
										 									m.addStatement(new ReturnSt(e4.toString()));
//										 									if (className.contains(".")) {
...
//		 		System.out.println(node.toString());
		 		...
		 		c.setPackageName(node.getName().toString());
		 		return false;
		 	}
	 	...
	 	public boolean visit(TypeDeclaration node) {
	 		...
	 		if (superInterfaceTypes != null) {
	 			for (Object object : superInterfaceTypes) {
	 				c.setSuperClass(object.toString());
				}
	 		}
...
//		new ProgramInfo().getMethod(method);
		...
		path1 = "C:/Users/Felipe/Documents/workspace/source/src/Package_0/ClassId_0.java";
