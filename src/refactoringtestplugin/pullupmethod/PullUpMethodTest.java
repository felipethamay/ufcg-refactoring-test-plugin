...
public abstract class PullUpMethodTest extends RefactoringTest {

	protected static final String METHOD_TO_REFACTOR = "m_0";
	protected static final String CLASS_NAME = "ClassId_0";

//	protected static final String METHOD_TO_REFACTOR = "runTest";
//	protected static final String CLASS_NAME = "SubClass";
...
//	}
	...
	protected Refactoring getRefactoring(List<ICompilationUnit> icus) throws JavaModelException {
		...
		PullUpRefactoringProcessor processor = new PullUpRefactoringProcessor(members,
				JavaPreferencesSettings.getCodeGenerationSettings(members[0].getJavaProject()));
...
//			}
	...
	protected IType getType(List<ICompilationUnit> icus, String name) throws JavaModelException {
		name = "P2_0.Class2_0";
		for (ICompilationUnit cu : icus) {
			IType[] types = cu.getAllTypes();
			for (int i = 0; i < types.length; i++) {
				String fullyQualifiedName = types[i].getFullyQualifiedName();
				if (fullyQualifiedName.equals(name) || types[i].getElementName().equals(name))
					return types[i];
			}
