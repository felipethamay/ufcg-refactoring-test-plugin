...
public abstract class PullUpFieldTest_ extends RefactoringTest {
	...
	protected static final String CLASS_NAME = "B";
	...
	private IMember[] findMembers(List<ICompilationUnit> icus) throws JavaModelException {
		for (ICompilationUnit icu : icus) {
			for (IType type : icu.getTypes()) {
				if (type.getSuperclassName() != null && !type.getSuperclassName().equals("null")) {
		...
		throw new RuntimeException("Field " + FIELD_TO_REFACTOR + " was not found"); // TODO:
																						// better
																						// exception
	}
