...
public abstract class AddParameterTest extends RefactoringTest {
	...
	protected Refactoring getRefactoring(List<ICompilationUnit> icus) throws JavaModelException {
		...
		if (targetMethod == null) {
			...
			if (targetMethod == null) {
				className = "ClassId_2";
				targetMethod = findTargetMethod(icus, className);
				if (targetMethod == null) throw new RuntimeException("Method " + TARGET_METHOD_NAME + " was not found"); // TODO: better exception
			}
		}
		...
		for (ICompilationUnit icu : icus) {
			for (IType type : icu.getTypes()) {				
				for (IMethod method : type.getMethods()) {
					...
					if (method.getParameterNames().length == 0 && method.getElementName().equals(TARGET_METHOD_NAME) && type.getElementName().equals(className)) {
						return method;
					}
				}
			}
