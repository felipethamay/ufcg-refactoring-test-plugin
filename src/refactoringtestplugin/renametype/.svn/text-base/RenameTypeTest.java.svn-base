...
public abstract class RenameTypeTest extends RefactoringTest {
	protected final static String ORIGINAL_TYPE_NAME = "ClassId_1";
	protected final static String NEW_TYPE_NAME = "ClassId_2";
	...
	protected Refactoring getRefactoring(List<ICompilationUnit> icus) throws JavaModelException {
		...
		for (ICompilationUnit icu : icus) {
			for (IType type : icu.getTypes()) {
				if (type.getElementName().equals("ClassId_1")) {
					cu = icu;
					continue;
				}
