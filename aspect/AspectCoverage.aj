...
public aspect AspectCoverage {
	...
	private File logResult = new File("C:/Users/Felipe/Documents/resultados/aspectCoverage.txt");
...
//				System.out.println(input.readLine());
	...
	pointcut advice () :  execution (* *.*(..))  && !within(Aspect);
		...
		if (!affectedTests.contains(""+test)) {
			...
			for (int i = 0; i < size; i++) {
				...
				if (im.contains(className+"."+methodName)) {
					if (!coveredImpactedMethods.contains(im)) {
						coveredImpactedMethods.add(im);
						System.out.println("covered impacted method: "+im);
					}
